...
class SupportRepository {
  ...
  Future<Ticket> details(String id) async => Ticket.fromJson((await Api.get('$base/$id'))['data']);
  ...
  Future<Ticket> create({
    ...
    final fields = {'subject': subject, 'description': description, 'priority': priority, 'platform': platform};
  ...
  Future<Ticket> reply(String id, String message, {List<PendingAttachment> files = const []}) async {
    final res = files.isEmpty
        ? await Api.post('$base/$id/reply', body: {'message': message})
        : await Api.postForm('$base/$id/reply', await _form({'message': message}, files));
    return Ticket.fromJson(res['data']);
  }
  ...
  Future<void> askForConfirmation(String id) => Api.post('$base/$id/ask-confirmation');

  /// [accept] true = "Accept" (closes), false = "Reject" (reopens, optional reason).
  Future<void> verify(String id, {required bool accept, String? reason}) => Api.post(
        '$base/$id/verify',
        body: {'action': accept ? 'Accept' : 'Reject', if (reason != null && reason.isNotEmpty) 'reason': reason},
      );

  Future<void> delete(String id) => Api.delete('$base/$id');
  Future<void> markRead(String id) => Api.patch('$base/$id/read');

  Future<FormData> _form(Map<String, String> fields, List<PendingAttachment> files) async {
    final form = FormData();
